...
namespace MicroJ {
    ...
    public class Verbs {
        ...
        public Verbs() {
            ...
            //BEGIN should move to helper
            expressionMap["astext"] = new VerbWithRank((y) => {
                if (y is A<JString>) { return y; }
                var ret = new A<JString>(new long[] { y.GetCount(), 1});
                var yl = y as A<long>;
                var yd = y as A<double>;
                var ydc = y as A<decimal>;
                var yi = y as A<int>;
                //compiler optimizes this
                for(var i = 0; i < ret.Count; i++) {
                    if (yl!=null)
                        ret.Ravel[i] = new JString { str = yl.Ravel[i].ToString()  };
                    else if (yd!=null)
                        ret.Ravel[i] = new JString { str = yd.Ravel[i].ToString()  };
                    else if (ydc!=null)
                        ret.Ravel[i] = new JString { str = ydc.Ravel[i].ToString()  };
                    else if (yi!=null)
            ...
            expressionMap["asint"] = new VerbWithRank((y) => {
                if (y is A<long>) { return y; }
                var ret = new A<long>(new long[] { y.GetCount()});
                var yl = y as A<long>;
                var yd = y as A<double>;
                var ydc = y as A<decimal>;
            ...
            expressionMap["left"] = new VerbWithRank((y) => {
                var yb = y as A<Box>;
                var ys = expressionMap["astext"].MonadicFunc(yb.Ravel[0].val) as A<JString>;
                var n = (int) yb.Ravel[1].val.GetLong(0);
                var ret = new A<JString>(new long[] { ys.GetCount(), 1});
                for(var i =0; i < ret.Count;i++) {
                    var str = ys.Ravel[i].str;
                    if (str.Length > n) { str = str.Substring(0,n); }
                    ret.Ravel[i] = new JString { str = str };
                }
                return ret;
            }, null, VerbWithRank.Infinite,  VerbWithRank.Infinite, VerbWithRank.Infinite);

            expressionMap["right"] = new VerbWithRank((y) => {
                var yb = y as A<Box>;
                var ys = expressionMap["astext"].MonadicFunc(yb.Ravel[0].val) as A<JString>;
                var n = (int) yb.Ravel[1].val.GetLong(0);
                var ret = new A<JString>(new long[] { ys.GetCount(), 1});
                for(var i =0; i < ret.Count;i++) {
                    var str = ys.Ravel[i].str;
                    if (str.Length > n) { str = str.Substring(str.Length-n,n); }
                    ret.Ravel[i] = new JString { str = str };
                }
                return ret;
            }, null, VerbWithRank.Infinite,  VerbWithRank.Infinite, VerbWithRank.Infinite);

            expressionMap["trim"] = new VerbWithRank((y) => {
                var ys = y as A<JString>;
                var ret = new A<JString>(new long[] { ys.GetCount(), 1});
                for(var i =0; i < ret.Count;i++) {
                    var str = ys.Ravel[i].str;
            ...
            expressionMap["upper"] = new VerbWithRank((y) => {
                var ys = y as A<JString>;
                var ret = new A<JString>(new long[] { ys.GetCount(), 1});
                for(var i =0; i < ret.Count;i++) {
                    var str = ys.Ravel[i].str;
            ...
            expressionMap["aggregate"] = new VerbWithRank(aggregate, null, VerbWithRank.Infinite,  VerbWithRank.Infinite, VerbWithRank.Infinite);
            expressionMap["addcol"] = new VerbWithRank(y=> {
                ...
                if (yb.Ravel[0].val is A<JTable>) {
                    ...
                    var newT = linktable(yb.Ravel[0].val as A<JTable>,ybt);
                    //replace current table with new table;
                    ybt.Ravel[0] = newT.Ravel[0];
                    return ybt;
                }
                ...
                options.Ravel[1].val = new JString { str = expression }.WrapA();

                var yta = yb.Ravel[yb.GetCount()-1].val as A<JTable>;
                var yt = yta.Ravel[0];
                ...
                yt.Calculations.Add(expression);
                yt.Dirty = true;

                //copy it back since it's a struct
                yta.Ravel[0] = yt;
                return yta;
            }, null, VerbWithRank.Infinite,  VerbWithRank.Infinite, VerbWithRank.Infinite);
            ...
            expressionMap["dedupe"] = new VerbWithRank(y => {
                //dedupe ('a';(3!:102) ((<'a');(1,2,1)))
                var yb = y as A<Box>;
                var ys = yb.Ravel[0].val as A<JString>;
                var yt = (yb.Ravel[1].val as A<JTable>).Ravel[0];
                ...
                if (keyss!=null) {                   
                    ...
                    for(var i = 0; i<rowCt;i++) {
                        var current = keyss.Ravel[i].str;
                        if (!idx.Contains(current)) {
                            idx.Add(current);
                            newIndices.Add(i);
                        }
                    }
                    Parser.Log("after hash",sw,reset:true);
                }
                else if (keysl != null) {
                    var idx = new HashSet<long>();
                    for(var i = 0; i<rowCt;i++) {
                        var current = keysl.Ravel[i];
                        if (!idx.Contains(current)) {
                            idx.Add(current);
                            newIndices.Add(i);
                        }
                    ...
                    throw new ApplicationException("cannot sort type of " + keys.GetType().ToString());
                }
            ...
            expressionMap["sanitizeCols"] = new VerbWithRank(y => {
                var yta = (y as A<JTable>);
                var yt = yta.First();
                var collisions = new Dictionary<string, int>();
                for(var i = 0; i < yt.Columns.Length;i++) {
                    var col = yt.Columns[i];
                    col = col.Replace("#", "Num").Replace("$", "Amt").Replace("%", "Pct");
                    col = Regex.Replace(col, "[^0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ]","");
                    if (!collisions.ContainsKey(col)) {
                        collisions[col] = 0;
                    }
                    collisions[col] = collisions[col] + 1;
                    yt.Columns[i] = col + ((collisions[col]>1) ? (collisions[col]).ToString() : "");
                }
                yta.Ravel[0] = yt;
                return yta;
            }, null, VerbWithRank.Infinite,  VerbWithRank.Infinite, VerbWithRank.Infinite);

            expressionMap["filter"] = new VerbWithRank(y => {
                var yb = (y as A<Box>);
                var yta = (yb.Ravel[1].val as A<JTable>);
                var expression = yb.Ravel[0].val.GetString(0);
                expression = expression.Replace("\"", "'");
                var newT = fromtable(new JString { str = expression }.WrapA() as A<JString>, yta).First();
                return copyTable(newT.WrapA());
            }, null, VerbWithRank.Infinite,  VerbWithRank.Infinite, VerbWithRank.Infinite);

            expressionMap["ifs"] = new VerbWithRank(y => {
                var yb = (y as A<Box>);
                if (yb.Count % 2 == 0) {
                ...
                for(var i = 0; i < ct; i++) {                    
                    ...
                    for(var k = 0; k < yb.Count-2; k=k+2) {
                        ...
                        else if (ints[k]!=null && ints[k].Ravel[i]==1) {
                            which[i] = k+1;
                            break;
                        }
                    }
        ...
        public AType aggregate(AType y) {
            var yb = y as A<Box>;
                ...
                Parser.Log("before sort",sw,reset:true);
                Array.Sort<long, long>(keys, indices);
                ...
                for(var k = 0; k < expressions.Ravel.Length; k++) {
                    ...
                    if (colvalsd != null && (op == "+/" || op == "#" || op == "{.")) {
                        ...
                        double[] dresult = new double[keyCt];
                        ...
                        outputRows[(k+1)] = new A<double>(keyCt) { Ravel = dresult }.Box();
                    }
                    else if (colvalss != null && (op == "{.")) {
                        JString[] sresult = new JString[keyCt];
                        //explicit ops are better than a conditional within
                        if (op == "{.") {
                            for(var i = 1; i < keys.Length;i++) {
                                if (keys[i]!=keys[i-1])
                        ...
                        outputRows[(k+1)] = new A<JString>(new long[] { keyCt, 1}) { Ravel = sresult }.Box();
                    }
                    else {
                        for(var i = 0; i < keys.Length; i++) {
                            if (lastKey != keys[i]) {
        ...
        public A<T> curtail<T>(A<T> y) where T : struct {
            ...
            if (vs != null && vs.GetCount() == 1) {
                ...
                if (str.Length <= 1) {
                    return (new JString { str = "" }.WrapA()) as A<T>;
                }
                str = str.Substring(0, str.Length-1);
                return (new JString { str = str }.WrapA()) as A<T>;
            }
            long[] newShape = y.ShapeCopy();
            newShape[0] = newShape[0] - 1;
            var v = new A<T>(newShape);
        ...
        public A<T> take<T>(A<long> x, A<T> y) where T : struct {
            ...
            if (y.GetType() == typeof(A<JString>) && y.Rank < 2) {
                ...
                //todo: move to own
                var yt = (y as A<JTable>).First();
                var take = Math.Min(xct, yt.RowCount);
                var offset = 0L;
                var xv = x.Ravel[0];
                if (xv < 0) {
                ...
                if ((yt.UniqueKeys != null || yt.Key != null) && xb.First().val.GetType() == typeof(A<Box>) && xb.First().val.GetCount() == 1) {
                    ...
                    for(var k = 0; k < idx.Length; k++) {
                        ...
                        if (i != -2) {
                            ...
                            if (suppliedColName.Contains(Parser.COLUMN_MASK_KEYWORD)) {
                                colName = suppliedColName.Split(new string[] { Parser.COLUMN_MASK_KEYWORD }, StringSplitOptions.RemoveEmptyEntries)[0].Trim();
                            }
                            columns.Add(colName);

                            if (suppliedColName.Contains(Parser.COLUMN_MASK_KEYWORD)) {
                                //functionto mask values
                                var orig = v.Rows[i].val as A<JString>;
                                var shape = orig.ShapeCopy();
                                if (shape.Length == 1) {
                                    shape = new long[] { v.RowCount, 1 };
                                }
                                var newVals = new A<JString>(shape);
                                var maskDict = new Dictionary<string, JString>();
                                var uniques = 0;
                                for (var n = 0; n < v.RowCount; n++) {
                                    var val = orig.GetString(n);
                                    if (!maskDict.ContainsKey(val)) {
                                        maskDict[val] = new JString { str = uniques.ToString() };
                                        uniques++;
                                    }
                                    newVals.Ravel[n] = maskDict[val];
                                }
                                rows.Add(newVals.Box());
                            }
                            else {
                                rows.Add(v.Rows[i]);
                            }
        ...
        public A<JTable> uniontable(A<JTable> x, A<JTable> y) {
            ...
            for (var i = 0; i < z.Columns.Length; i++) {
                ...
                if (colb >= 0) {
                    ...
                    AType newRow = xv.Rows[i].val.Clone(yv.RowCount);
                    if (xv.Rows[i].val is A<JString>) {
                        var empty = new JString { str = "" };
                        for(var k = xv.RowCount;k<(int)xv.RowCount+yv.RowCount;k++) {
                            newRow.SetVal(k,empty);
                        }
                    }
                    z.Rows[i] = newRow.Box();
                }
        ...
        public AType symbolizeDyad(AType x, AType y) {
            ...
            //write symbl enumeration to binary --- 10 s: ('DB';'Region';(1{syms))
            ...
            if (op == "]") {
                ...
                */
                ...
                if (y.GetType() == typeof(A<JString>) && y.IsAtom()) {
                    ...
                    else if (Parser.Names.ContainsKey(parts[0]) || str.Contains("=:") || str.Contains("=.") || str.Contains("smoutput")) { //eval.. make a smarter check to see if it's all numbers
                        return Parser.exec(str);
                    }
                }
                return toNumbers(y);
            }
    ...
    public class Conjunctions {
        ...
        //' ' (150!:0) 'System.Diagnostics.Debugger.Break();return AType.MakeA(0);'
        ...
        public AType execParallelMap(A<Box> x, A<Box> y, string fromVerb = null, AType op = null) {
            ...
            var table = y.Ravel.Length > 1 ? y.Ravel[1].val.GetString(0).Trim() : "";
            string column = null;
            A<Box> xoptions = null;
        ...
        public Tuple<Stopwatch, List<double>> measureTime(Tuple<Stopwatch, List<double>> measurement, string msg = null) {
#if DEBUG
            Stopwatch watch;
            List<double> times;
            ...
            if (!watch.IsRunning) {
                ...
                if (msg != null) {
                    Parser.Log(msg + " took " + watch.ElapsedMilliseconds);
                }
                watch.Start();
            }
        ...
        public AType readcsv(A<Box> xb, A<Box> y) {
            ...
            bool noPad = true; //optionsDict.ContainsKey("nopad");
            if (optionsDict.ContainsKey("pad")) {
                noPad = false;
            }
                    ...
                    for (var k = 0; k < fieldCount; k++) {
                        ...
                        if ((types[k] == 0 || types[k] == 3) && Int32.TryParse(csv[k], out n)) {
                            ...
                            if (types[k] != TYPE_STR && types[k] != 0) {
                                Debug.WriteLine("Line: " + iLine + "| Promoting " + headers[k] + " to string " + csv[k]);
                            }
#endif
                            types[k] = TYPE_STR;
                        }
        ...
        public AType boxToJSON(A<Box> y) {
            ...
            for(var i = 0; i < y.Count; i++) {
                var tbl = y.Ravel[i].val as A<JTable>;
                var jstr = y.Ravel[i].val as A<JString>;

                if (tbl != null) {
                    dto.Add(tbl.First().ToJson());
                }
                else if (jstr != null) {
                    dto.Add(y.Ravel[i].val.GetString(0));
                }
                else {
        ...
        public AType namesToJSON(A<JString> y) {
            var match = y.GetString(0);
            var serializer = new JavaScriptSerializer();
            serializer.MaxJsonLength = Int32.MaxValue;
            var dto = new Dictionary<string, object>();
            Action<Dictionary<string, AType>> ser = (names) => {
                if (names == null) { return; }
                foreach (var kv in names.Where(x => x.Key.Contains(match))) {
                    var tbl = kv.Value as A<JTable>;
                    var jstr = kv.Value as A<JString>;
                    var larr = kv.Value as A<long>;
                    var darr = kv.Value as A<double>;
                    var marr = kv.Value as A<Decimal>;

                    if (tbl != null) {
                        dto[kv.Key] = tbl.First().ToJson();
                    }
                    else if (jstr != null) {
                        dto[kv.Key] = jstr.Ravel.Select(x => x.str).ToArray();
                    }
                    else if (larr != null) {
                        dto[kv.Key] = larr.Ravel;
                    }
                    else if (darr != null) {
                        dto[kv.Key] = darr.Ravel;
                    }
                    else if (marr != null) {
                        dto[kv.Key] =  marr.Ravel;
                    }
                    else {
                        dto[kv.Key] = y.GetString(0);
                    }
                }
            };
            ser(Parser.LocalNames);
            ser(Parser.Names);
        ...
        public unsafe AType readTableText(A<Box> x, A<Box> y, string column = null) {
            var optionsDict = new Dictionary<string, string>();
            ...
            {
                limit = int.Parse(optionsDict["limit"]);
            }
            ...
            string[] onlyCols = null;

            if (optionsDict.ContainsKey("onlyCols")) {
                onlyCols = optionsDict["onlyCols"].Split(',');
                files = files.Where(file => onlyCols.Contains(file.Name.Replace(ext, ""))).ToArray();
            }

            var cols = files.Select(file => file.Name.Replace(ext, ""));

            var rowBoxes = files.AsParallel().Select(file => {
                ...
                if (!noDetect) { //detect columns
                    using (var sr = new StreamReader(fileName)) {
			            sr.ReadLine();
			            isInt = true;
			            int sample = 0;
			            string line = null;
			            while (sample < 10000 && (line = sr.ReadLine()) != null) {
                        if (unquote) { line = line.Trim('\"'); }
			                if ((sample % 3) == 0) {
				                ...
				                else if (!Double.TryParse(line, out dval)) {
				                    isDouble = false;
				                    break;
				                }
			                }
                ...
                using (var sr = new StreamReader(fileName)) {
		            ...
		            if (isInt) {
                        ...
                        if ((!long.TryParse(line, out lval) && line != "") ||
		                        (line.StartsWith("0") && !line.Contains(".") && line.Length > 1)) {
		                        isInt = false;
                                if (double.TryParse(line, out dval)) {
                                    isDouble = true;
                                } else {
                                    isDouble = false;
                                }
		                        Console.WriteLine("Line: " + count + "| PROMOTING " + fileName + " to " + (isDouble?"double":"string") + " due to:" + line);
                        ...
                        if (isInt) {
	                        var dvals = new A<long>(new long[] { drows.Count }) { Ravel = drows.ToArray() };
	                        return dvals.Box();
                        }
		            }
                    count = 0;
                    if (isDouble) {
			            var drows = new List<double>();
			            while (count < limit && (line = sr.ReadLine()) != null) {
                        if (unquote) { line = line.Trim('\"'); }
                        double dval = 0;
                        ...
                        if ((!Double.TryParse(line, out dval) && line != "") ||
		                        (line.StartsWith("0") && !line.Contains(".") && line.Length > 1)) {
		                        isDouble = false;
		                        Console.WriteLine("Line: " + count + "| PROMOTING " + fileName + " to string due to:" + line);
		                        sr.DiscardBufferedData();
		                        sr.BaseStream.Seek(0, System.IO.SeekOrigin.Begin);
		                        sr.ReadLine(); //read header
		                        break;
                        ...
                        if (isDouble) {
	                        var dvals = new A<double>(new long[] { drows.Count }) { Ravel = drows.ToArray() };
	                        return dvals.Box();
                        }
		            }
                    count = 0;
                    ...
                    //sr.ReadLine() will return null if the last line is a newline
                    //determine if that was the case
                    if (count != limit) {
                        sr.BaseStream.Seek(-1,SeekOrigin.End);
                        if (sr.BaseStream.ReadByte() == '\n') {
        ...
        public unsafe AType readTableBinary(A<Box> x, A<Box> y, string column = null) {
            ...
            bool stringsAsBytes = optionsDict.ContainsKey("stringsAsBytes");
            bool unmask = optionsDict.ContainsKey("unmask");
            bool txtTable = optionsDict.ContainsKey("txtTable");
            bool createDir = optionsDict.ContainsKey("createDir");

            if (optionsDict.ContainsKey("cacheVar")) {
                var cacheVar = optionsDict["cacheVar"];
                if (Parser.Names.ContainsKey(cacheVar) && Parser.Names[cacheVar] is A<JTable> ) {
                    Parser.Log("loading " + cacheVar + " from cache");
                    return Parser.Names[cacheVar];
                }
            }
            if (txtTable) {
                return readTableText(x, y, column);
            }
            var path = y.First().val.GetString(0);

            string ext = !txtTable ? ".bin" : ".txt";
            ...
            if (column != null) {
                files = new DirectoryInfo(path).GetFiles("*" + column + ext);
            }
            ...
            var skipCols = new string[] { };
            string[] onlyCols = null;

            if (optionsDict.ContainsKey("skipCols")) {
                skipCols = optionsDict["skipCols"].Split(',');
            }
            if (optionsDict.ContainsKey("onlyCols")) {
                onlyCols = optionsDict["onlyCols"].Split(',');
            }
            ...
            bool skipSyms = false;
            if (optionsDict.ContainsKey("autosym")) {
                string symfile = Path.Combine(path, "sym.bin");
                if (File.Exists(symfile)) {
                    optionsDict["symfile"] = symfile;
                } else {
                    skipSyms = true;
                }
            }
                ...
                using (var fs = new FileStream(symfile, FileMode.Open, FileAccess.Read)) {
                    using (var bw = new BinaryReader(fs)) {
                        ...
                        for (var i = 0; i < ct; i++) {
                            var str = bw.ReadString();
                            long symid = -1 * (i + 1);
                            //TODO: figure out if we need to intern strings
                            //-- I don't think so since there's only one copy in string table
                            //symbolDict[symid] = new JString { str = String.Intern(str) };
                            symbolDict[symid] = new JString { str = str };}
            ...
            if (skipSyms) {
                files = files.Where(file => !file.Name.Contains(".sym.bin")).ToArray();
            }
            //holds symbol indices for quicker comparison
            var columnSymbols = new Dictionary<string, long[]>();
            ...
            foreach (var file in files) {
                ...
                var spec = parts[0];
                var col = String.Join("_", parts.Skip(1).ToArray()).Replace(ext, "");
                if (txtTable)
                {
                    col = file.Name.Replace(ext, "");
                }
                var num = file.Length;
                ...
                if (col.EndsWith(".sym") && !skipSyms) {
                    col = col.Replace(".sym", "");
                }
                if (skipCols.Contains(col)) {
                    continue;
                }
                if (onlyCols != null && !onlyCols.Contains(col)) {
                    continue;
                }
                ...
                if (txtTable)
                {
                    ...
                    foreach (var line in File.ReadLines(path + "\\" + file.Name).Skip(1)) {
                        rows.Add(new JString { str = String.Intern(line) });
                    }
                    var val = new A<JString>(new long[] { rows.Count, 1 }) { Ravel = rows.ToArray() };
                ...
                using (var mmf = MemoryMappedFile.CreateFromFile(path + "\\" + file.Name, FileMode.Open)) {
                    using (var view = mmf.CreateViewAccessor(0, num)) {
                        ...
                        AType val = null;
                        view.SafeMemoryMappedViewHandle.AcquirePointer(ref ptr);
                        if (spec.StartsWith("b") || (spec.StartsWith("s") && stringsAsBytes)) {
                            int size = Int32.Parse(spec.Substring(1));
                            var rows = size > 0 ? num / size : num;
                            byte[] arr = new byte[num];
                            System.Runtime.InteropServices.Marshal.Copy(IntPtr.Add(new IntPtr(ptr), 0), arr, 0, (int)num);
                            ...
                            var rows = num / sizeof(decimal);
                            decimal[] arr = new decimal[rows];

                            using (var br = new BinaryReader(mmf.CreateViewStream())) {
                                for (var i = 0; i < rows; i++) {
                                    arr[i] = br.ReadDecimal();
                                }
                            }
                            val = new A<decimal>() { Ravel = arr, Shape = new long[] { rows } };
...
#if DEBUG
                Parser.Log("reading " + path + "\\" + file, sw);
#endif
            }


            var table = new JTable {
                Columns = cols.Where(f=>f!=null).ToArray(),
            ...
            if (skipSyms) {
#if DEBUG
                Parser.Log("Writing symbols that were skipped" + path);
#endif
                Verbs.symbolizeTable(new JString { str = path }.WrapA() as A<JString>, table);

            }
#if DEBUG
            Parser.Log("DONE reading " + path, totalTime);
#endif
            return table;
        }
        ...
        public AType Call1(AType method, AType y) {
            ...
            //bond
                ...
                if (y.GetType() == typeof(A<JTable>)) {
                    ...
                    return boxToJSON((A<Box>) y);
                }
                else if (y.GetType() == typeof(A<JString>)) {
                    return namesToJSON((A<JString>)y);
                }
            }
                ...
                return new JTable { partitioned = y as A<Box> }.WrapA();
            }
        ...
        public AType Call2(AType method, AType x, AType y) {
            ...
            if (verb.conj == "!:" && verb.op == "150") {
                ...
                else if (x.GetType() == typeof(A<JTable>)) {
                    return calldotnet((A<JTable>)x, (A<JString>)y);
                }
            }
            else if (verb.conj == "\"") {
    ...
    public class Adverbs {
        ...
        public AType keyTable<T2, T>(AType op, A<T2> x, A<T> y) where T : struct where T2 : struct {
            ...
            var stopWatch = Conjunctions.measureTime(null, "before group");
            ...
            if (x != null && !x.IsInfinity()) {
                ...
                if (colIdx.Length == 1 && yt.Rows[colIdx[0]].val.GetType() == typeof(A<long>) && yt.indices == null) {
                    var col = yt.Rows[colIdx[0]].val as A<long>;
                    for (var i = 0; i < rowCt; i++) {
                        var key = col.Ravel[i];
                        List<long> match = null;
                        if (!lkeyIndices.TryGetValue(key, out match)) {
                            match = new List<long>();
                            lkeyIndices[key] = match;
                        }
                        match.Add(i);
                    }
                }
                else if (colIdx.Length == 1 && yt.Symbols != null && yt.Symbols.ContainsKey(yt.Columns[colIdx[0]]) && yt.indices == null) {
                    var syms = yt.Symbols[yt.Columns[colIdx[0]]];
                    for (var i = 0; i < rowCt; i++) {
                        var key = syms[i];
                        List<long> match = null;
                        if (!lkeyIndices.TryGetValue(key, out match)) {
            ...
            stopWatch = Conjunctions.measureTime(stopWatch, "after group");

            var keyCt = Math.Max(keyIndices.Count, lkeyIndices.Count);
            ...
            if (op.ToString() == "]") {
                ...
                var cols = colIdx != null ? colIdx.Select(xt=>yt.Columns[xt]).ToArray().Concat(expressionCols).ToArray() : expressionCols.ToArray();
                ...
                if (colIdx != null && colIdx.Length == 1 && keyIndices != null && keyIndices.Count > 0) {
                    ...
                    var idx = 0;
                    foreach(var kv in keyIndices) {
                        at.Ravel[idx] = new JString { str = kv.Key };
                        idx++;
                    }
                    rows[colOffset++] = at.Box();
                }
                else {
                ...
                for(var k = 0; k < expressions.Length;k++) {
                    ...
                    IEnumerable<long[]> keyIndexValues = null;
                    if (keyIndices.Count() > 0) { keyIndexValues = keyIndices.Values.ToArray().Select(z=>z.ToArray()); }
                    if (lkeyIndices.Count() > 0) { keyIndexValues = lkeyIndices.Values.ToArray().Select(z => z.ToArray()); }
                    foreach (var val in keyIndexValues) {
                        ...
                        if (!noParse) {
                            ...
                            long groupCt = 0;
                            locals["_N"] = rowCtA;
                            locals["_G"] = new A<long>(0) { Ravel = new long[] { val.Length } };
                            locals["_I"] = new A<long>(0) { Ravel = new long[] { groupCt++ } };
                        ...
                        if (noParse) {
                            for (var i = 0; i < yt.Columns.Length; i++) {
                                if (JTable.SafeColumnName(yt.Columns[i]) == expressionParts[1]) {
                                    locals[JTable.SafeColumnName(yt.Columns[i])] = allRows ? yt.Rows[i].val : yt.Rows[i].val.FromIndices(val, false);
                                }
                ...
                stopWatch = Conjunctions.measureTime(stopWatch, "after combine");
            ...
            if (options.ContainsKey("ukey")) {
               ...
               //todo add conversion from [Column] 
                try {
                    ...
                    if (yt.partitioned == null) {
                        var newT = newTable.Ravel[0];
                        //extend scalar to all rows
                        if (yt.RowCount != newT.RowCount && newT.RowCount == 1) {
                            var newRow = newT.Rows[0].val.Clone(yt.RowCount);
                            var indices = new long[yt.RowCount];
                            newRow = newRow.FromIndices(indices);
                            newT.Rows[0] = newRow.Box();
                        }
                        yt = Verbs.linktable(y, newTable).First();
                    }
                    ...
                    throw new ApplicationException("Error on addcol with " + expr, e);
                }
        ...
        public A<JTable> amendTable(AType noun, AType newVal, A<JTable> y, bool approx = false) {
            ...
            if (!approx) {
                ...
                for (var i = 0; i < yt.Rows[0].val.GetCount(); i++) {
                    var keyVal = yt.Rows[keyIdx].val.GetString(i).ToUpper().Trim(); //ensure trimmed case-insensitive
            ...
            if (newVal.GetType() != typeof(A<JTable>)) {
                for (var i = 0; i < newVal.GetCount(); i++) {
                    List<long> keyIndices = null;
                    var checkVal = yb.Ravel[1].val.GetString(i).ToUpper().Trim(); //ensure trimmed case-insensitive
                ...
                string lookupCol = yb.Count >= 3 && !noOrig && yb.Ravel[2].val.ToString() != "*" ? yb.Ravel[2].val.ToString() : null;
                AType missingVal = yb.Count >= 4 ? yb.Ravel[3].val : null;
                var joinKeyIdx = Array.IndexOf(xt.Columns, keyColumn);
                if (joinKeyIdx == -1) {
                    throw new ApplicationException("join column not found: " + keyColumn);
                }
                if (approx) {
                ...
                for (var i = 0; i < xt.RowCount; i++) {
                    var keyVal = xt.Rows[joinKeyIdx].val.GetString(i).ToUpper().Trim(); //ensure trimmed case-insensitive
                    joinKeys[keyVal] = i;
                }
                var newCols = xt.Columns.Where(xv => xv != keyColumn && (xv == lookupCol || lookupCol == null)).ToArray();
                var newRows = new Box[newCols.Length];
                if (newCols.Length == 0) {
                    throw new ArgumentException("Could not find or key off of " + lookupCol);
                }
                for (var i = 0; i < newCols.Length; i++) {
                    ...
                    newRows[i] = xt.Rows[xtIdx].val.FromIndices(indices,true,false,missingVal).Box();
